...
class EvernoteIndexer {
            ...
            for (const srcTypeStr in typeConfig.denormalizedQueries) {
                ...
                for (const queryName in queries) {
                    ...
                    this.traversalConfig[srcType][traversalName] = Object.assign({ srcType,
                        dstType, dstQuery: queryName, dstConstraint: queryConfig.traversalConstraint || conduit_storage_1.EdgeConstraint.MANY }, queryConfig);
                }
            }
        }
    }
    ...
    isContainerizedIndexComponent(key, definition) {
        if (definition.inputs && Object.keys(definition.inputs).includes(key)) {
            if (definition.inputs[key].constraint !== conduit_storage_1.EdgeConstraint.MANY) {
                return 'inputs';
            }
            else {
        ...
        if (definition.outputs && Object.keys(definition.outputs).includes(key)) {
            if (definition.outputs[key].constraint !== conduit_storage_1.EdgeConstraint.MANY) {
                return 'outputs';
            }
            else {
                throw new Error(`Cannot index directly on a MANY constraint port, index on ${key}Count instead`);
    ...
    chooseBestIndex(indexesArray, filters, sortParams, additionalParams) {
        const indexesWithParams = conduit_storage_1.indexesWithAllParams(indexesArray, filters, sortParams.concat(additionalParams));
        if (!indexesWithParams.length) {
            const allParams = filters.map(f => f.field).concat(sortParams).concat(additionalParams);
            return conduit_storage_1.indexWithTheMostParams(indexesArray, allParams);
        }
        let best;
        for (const index of indexesWithParams) {
            const unconditionalFilterParams = filters.filter(f => !conduit_storage_1.indexHasConditionFilterParam(index, f)).map(f => f.field);
    ...
    async getIterator(trc, watcher, tree, type, indexItem, filters, sorts, reverseOrder, pageInfo) {
        ...
        if (this.keyIsRanged(firstKeyInSet)) {
            ...
            iterator = new conduit_storage_1.IndexingIterator(trc, watcher, this.keyFilterFactory(type, indexItem.index, filters), tree, iterationDirection === 'ASC', firstKey, lastKey, startKey !== null && startKey !== void 0 ? startKey : undefined, pageInfo === null || pageInfo === void 0 ? void 0 : pageInfo.pageSize);
        }
        else {
            const startKey = await getStartKey(trc, watcher, pageInfo, tree, firstKeyInSet, lastKeyInSet, iterationDirection);
            iterator = new conduit_storage_1.IndexingIterator(trc, watcher, this.keyFilterFactory(type, indexItem.index, filters), tree, iterationDirection === 'ASC', firstKeyInSet, lastKeyInSet, startKey !== null && startKey !== void 0 ? startKey : undefined, pageInfo === null || pageInfo === void 0 ? void 0 : pageInfo.pageSize);
        }
        return iterator;
    }
