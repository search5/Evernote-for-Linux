...
async function getShareAllowance(context) {
    ...
    const cacheFillFunc = async (node, syncContext) => {
        ...
        await context.db.transactSyncedStorage(context.trc, 'cacheShareAllowance', async (tx) => {
            await tx.setNodeCachedField(context.trc, en_core_entity_types_1.ACCOUNT_LIMITS_REF, 'noteAndNotebookSharesAllowance', ret, { 'Counts.userNoteAndNotebookSharesSentCount': node.NodeFields.Counts.userNoteAndNotebookSharesSentCount });
        });
        return ret;
    };
    const { node: user } = await context.db.getNodeWithContext(context, { id: conduit_core_1.PERSONAL_USER_ID, type: en_core_entity_types_1.CoreEntityTypes.User });
    if (!user) {
    ...
    if (user && user.NodeFields.created && (Date.now() - user.NodeFields.created) < conduit_utils_1.MILLIS_IN_SEVEN_DAYS) {
        const limitRef = { id: en_core_entity_types_1.ACCOUNT_LIMITS_ID, type: en_core_entity_types_1.CoreEntityTypes.AccountLimits };
...
function ShareAllowanceResolver() {
    return {
        'AccountLimits.noteAndNotebookSharesAllowance': {
            ...
            resolve: async (nodeRef, _, context) => {
                return getShareAllowance(context);
            },
        },
    };
}
