...
async function getShares(trc, noteStore, authToken, nodeRef) {
    switch (nodeRef.type) {
        case en_core_entity_types_1.CoreEntityTypes.Note:
            return noteStore.getNoteShares(trc, authToken, en_thrift_connector_1.convertGuidToService(nodeRef.id, en_core_entity_types_1.CoreEntityTypes.Note));
        case en_core_entity_types_1.CoreEntityTypes.Notebook:
            return noteStore.getNotebookShares(trc, authToken, en_thrift_connector_1.convertGuidToService(nodeRef.id, en_core_entity_types_1.CoreEntityTypes.Notebook));
        default:
...
async function processAndCacheAcceptedMemberships(trc, tx, response, parentEdge) {
    ...
    if (response.memberships) {
        for (const member of response.memberships) {
            out[en_thrift_connector_1.convertGuidFromService(member.recipientUserId, en_core_entity_types_1.CoreEntityTypes.Profile, en_core_entity_types_1.PROFILE_SOURCE.User)] = true;
        }
    }
    if (response.invitations) {
        for (const invite of response.invitations) {
            ...
            out[en_thrift_connector_1.convertGuidFromService(profileID, en_core_entity_types_1.CoreEntityTypes.Profile, en_core_entity_types_1.PROFILE_SOURCE.Identity)] = false;
        }
    }
