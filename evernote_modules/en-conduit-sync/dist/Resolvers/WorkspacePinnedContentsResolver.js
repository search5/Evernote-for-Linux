...
function WorkspacePinnedContentsResolver() {
    async function resolveWorkspacePinnedContents(workspaceID, context) {
        ...
        const res = await context.db.getNodeCachedField(context, { id: workspaceID, type: en_core_entity_types_1.CoreEntityTypes.Workspace }, en_thrift_connector_1.PINNED_CONTENTS_FIELD_NAME, async (node, syncContext) => {
            ...
            const pinnedContentsFromServer = await utilityStore.listPinnedContent(context.trc, auth.token, en_thrift_connector_1.convertGuidToService(node.id, en_core_entity_types_1.CoreEntityTypes.Workspace));
            const pinnedContents = [];
            for (const pContent of pinnedContentsFromServer) {
                const convertResult = en_thrift_connector_1.convertPinnedContentFromService(pContent);
                if (convertResult) {
            ...
            await context.db.transactSyncedStorage(context.trc, 'WorkspacePinnedContentsResolver', async (tx) => {
                await tx.setNodeCachedField(context.trc, node, en_thrift_connector_1.PINNED_CONTENTS_FIELD_NAME, pinnedContents, {});
            });
