...
const DemandFetchNoteActivity_1 = require("./DemandFetchNoteActivity");
...
class ENSyncManager extends conduit_core_1.SyncManager {
    constructor(di, thriftComm, syncEngine) {
        ...
        this.syncStateChangeMutex = new conduit_utils_1.Mutex('SyncManager syncStateChange', 10000);
    ...
    async destructor(trc) {
        ...
        await this.syncStateChangeMutex.destructor();
    }
    getAuth() {
        return this.auth;
    }
    ...
    async initAuth(trc, auth, startPaused) {
        ...
        await this.pauseSyncing(trc);
        await SyncHelpers_1.clearSyncProgress(trc, this.activityContext.syncEngine);
        if (auth) {
            ...
            if (!syncInitialized) {
                ...
                if (this.di.downsyncConfig.downsyncMode === conduit_view_types_1.DownsyncMode.LEGACY_FOR_PREBUILT) {
                    if (isColdStart) {
                        // we want NSyncInitActivity has higher priority than IncrementalSyncActivity and NSyncInitialDownsyncActivity so they can pull data from nsync.
                        activities.push(this.getNSyncInitActivity(trc, en_conduit_sync_types_1.SyncActivityPriority.INITIAL_DOWNSYNC));
                        ...
                        activities.push(new HybridInitialDownsyncActivity_1.NSyncInitialDownsyncActivity(this.di, this.activityContext, HybridInitialDownsyncActivity_1.FINAL_ACTIVITY_ORDER.NSYNC_ONLY_INCREMENTAL_SYNC));
                    }
                }
                else {
                    // have NSyncInitActivity not running behind the loading screen to clear the loading screen asap.
                    activities.push(this.getNSyncInitActivity(trc, en_conduit_sync_types_1.SyncActivityPriority.IMMEDIATE));
                    if (isColdStart) {
                        activities.push(this.getSchemaMigrationActivity(trc));
                        activities.push(this.getReindexingActivity(trc));
                    }
                    ...
                    // activities.push(new DemandFetchNoteActivity(this.di, this.activityContext, null));
                }
            }
    ...
    async pauseSyncing(trc) {
        await this.syncStateChangeMutex.runInMutex(trc, 'syncManager.pauseSyncing', async () => {
    ...
    async resumeSyncing(trc) {
        await this.syncStateChangeMutex.runInMutex(trc, 'syncManager.resumeSyncing', async () => {
    ...
    async disableSyncing(trc) {
        await this.syncStateChangeMutex.runInMutex(trc, 'syncManager.disableSyncing', async () => {
    ...
    async enableSyncing(trc) {
        await this.syncStateChangeMutex.runInMutex(trc, 'syncManager.enableSyncing', async () => {
    ...
    async toggleEventSync(trc, disable) {
        ...
        await this.syncStateChangeMutex.runInMutex(trc, 'syncManager.toggleEventSync', async () => {
    ...
    async addImmediateActivity(trc, activity, timeout, useExistingActivity = false) {
        let err = null;
        const lockname = `immediateAddActivity-${activity.params.activityType}`;
        if (!this.locks[lockname]) {
            this.locks[lockname] = new conduit_utils_1.Mutex(lockname);
        }
        if (useExistingActivity && this.locks[lockname].isLocked()) {
            const unlock = await this.locks[lockname].acquireLock(trc, 10000, lockname);
            await unlock();
            return;
        }
        const unlock = await this.locks[lockname].acquireLock(trc, 10000, lockname);
        try {
            ...
            if (!activity.params.subpriority) {
                // if subpriority not specified, make sure activity doesn't run before other higher priority activities.
                activity.params.subpriority = HybridInitialDownsyncActivity_1.FINAL_ACTIVITY_ORDER.IMMEDIATE_ACTIVITY;
            }
    ...
    async forceDownsyncUpdate(trc, timeout) {
        await this.addImmediateActivity(trc, new IncrementalSyncActivity_1.IncrementalSyncActivity(this.di, this.activityContext, en_conduit_sync_types_1.SyncActivityPriority.IMMEDIATE), timeout, true);
    }
    async forceNSyncFlush(trc) {
        var _a;
        if ((_a = this.activityContext.syncEventManager) === null || _a === void 0 ? void 0 : _a.requiresFlush()) {
            await this.addImmediateActivity(trc, new NSyncFlushActivity_1.NSyncFlushActivity(this.di, this.activityContext, en_conduit_sync_types_1.SyncActivityPriority.IMMEDIATE), null, true);
        }
    }
    ...
    async needContentFetchSync(trc) {
        ...
        const noteIDs = await en_thrift_connector_1.getPendingOfflineNoteIDs(trc, this.activityContext.syncEngine.graphStorage);
    ...
    async immediateDemandFetchNote(trc, args) {
        await this.addImmediateActivity(trc, new DemandFetchNoteActivity_1.DemandFetchNoteActivity(this.di, this.activityContext, args), null);
        // TODO: [CON-2401] Enable once we want this as a background activity
        // await this.addActivity(trc, new DemandFetchNoteActivity(this.di, this.activityContext, SyncActivityPriority.BACKGROUND));
    }
    ...
    getNSyncInitActivity(trc, priority) {
        return new NSyncInitActivity_1.NSyncInitActivity(this.di, this.activityContext, priority, HybridInitialDownsyncActivity_1.FINAL_ACTIVITY_ORDER.NSYNC_INIT_ACTIVITY);
    }
    ...
    async getContentFetchSyncActivity(trc) {
        ...
        await en_thrift_connector_1.resetOfflineNbsSyncStateOnInit(trc, this.activityContext.syncEngine.graphStorage);
    ...
    isBackgroundNoteSyncFinished(trc) {
        return this.findActivityIndex(trc, en_conduit_sync_types_1.SyncActivityType.BackgroundNoteSyncActivity) === -1;
    }
}
__decorate([
