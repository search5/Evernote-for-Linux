...
async function updateOfflineSyncState(trc, params, noteID, syncContext, resourceID, op) {
    return await params.syncEngine.transact(trc, 'updateOfflineSyncState', async (tx) => {
        ...
        const origNoteSyncState = await en_thrift_connector_1.getPendingOfflineNote(trc, tx, noteID);
            ...
            case UpdateSyncStateOperation.RESOURCE_NEEDS_DOWNLOAD:
                if (resourceID && noteSyncState.resources && noteSyncState.resources[resourceID]) {
                    if (op === UpdateSyncStateOperation.RESOURCE_DONE) {
                        ...
                        noteSyncState = simply_immutable_1.replaceImmutable(noteSyncState, ['resources', resourceID], (op === UpdateSyncStateOperation.RESOURCE_IN_PROGRESS) ? en_thrift_connector_1.OfflineEntityDownloadState.IN_PROGRESS : en_thrift_connector_1.OfflineEntityDownloadState.NEEDS_DOWNLOAD);
                    }
                }
                break;
            default:
        ...
        if (!doDelete && !noteSyncState.fetchContent && !noteSyncState.resources) {
            ...
            await en_thrift_connector_1.updateContentDownloadForNote(trc, tx, syncContext, noteID, true);
            doDelete = true;
        }
        if (doDelete) {
            await en_thrift_connector_1.deletePendingOfflineNoteSyncState(trc, tx, noteID);
            return true;
        }
        else if (noteSyncState !== origNoteSyncState) {
            await en_thrift_connector_1.updatePendingOfflineNoteSyncState(trc, tx, noteID, noteSyncState);
        }
        return false;
    });
}
async function fetchNoteContent(trc, params, note, syncContext) {
    ...
    try {
        ...
        const noteStore = params.comm.getNoteStore(authData.urls.noteStoreUrl);
        ...
        await graphStorage.transact(trc, 'noteContentToGraph', async (graphTransaction) => {
            ...
            await en_thrift_connector_1.updateBlobToGraph(trc, graphTransaction, contentBlobData, { id: noteID, type: en_core_entity_types_1.CoreEntityTypes.Note }, 'content', syncContext);
            for (const resource of (serviceData.resources || [])) {
                const resourceRef = { id: en_thrift_connector_1.convertGuidFromService(resource.guid, en_core_entity_types_1.CoreEntityTypes.Attachment), type: en_core_entity_types_1.CoreEntityTypes.Attachment };
                if (resource.data) {
                    const resourceUrl = en_thrift_connector_1.generateResourceUrl(metadata, 'res', resource.guid);
                    const resourceBlobData = Object.assign(Object.assign({}, resource.data), { url: resourceUrl });
                    await en_thrift_connector_1.updateBlobToGraph(trc, graphTransaction, resourceBlobData, resourceRef, 'data', syncContext);
                }
                if (resource.recognition) {
                    await en_thrift_connector_1.updateBlobToGraph(trc, graphTransaction, resource.recognition, resourceRef, 'recognition', syncContext);
                }
                if (resource.alternateData) {
                    await en_thrift_connector_1.updateBlobToGraph(trc, graphTransaction, resource.alternateData, resourceRef, 'alternateData', syncContext);
                }
            }
        });
...
async function fetchNoteResources(trc, params, noteID, syncContext, maxResources) {
    const graphStorage = params.syncEngine.graphStorage;
    const noteSyncState = await en_thrift_connector_1.getPendingOfflineNote(trc, graphStorage, noteID);
    if (!noteSyncState) {
        return true;
    }
    ...
    const searchTextAllowed = en_thrift_connector_1.isSearchTextAllowed(userNode);
    for (const iterResourceID in noteSyncState.resources) {
        const resourceID = iterResourceID;
        if (noteSyncState.resources[resourceID] === en_thrift_connector_1.OfflineEntityDownloadState.NEEDS_DOWNLOAD) {
...
async function downsyncNote(trc, params, noteID, maxResources) {
    ...
    let noteSyncState = await en_thrift_connector_1.getPendingOfflineNote(trc, graphStorage, noteID);
    if (!noteSyncState) {
        return;
    }
    ...
    if (noteSyncState.needsInit) {
        ...
        for (const key in note.outputs.attachments) {
            noteSyncState.resources[note.outputs.attachments[key].dstID] = en_thrift_connector_1.OfflineEntityDownloadState.NEEDS_DOWNLOAD;
        }
        for (const key in note.outputs.inactiveAttachments) {
            noteSyncState.resources[note.outputs.inactiveAttachments[key].dstID] = en_thrift_connector_1.OfflineEntityDownloadState.NEEDS_DOWNLOAD;
        }
...
async function fetchAndUpdateSearchText(trc, params, attachment, authData) {
    ...
    const noteStore = params.comm.getNoteStore(authData.urls.noteStoreUrl);
    const serviceGuid = en_thrift_connector_1.convertGuidToService(attachment.id, en_core_entity_types_1.CoreEntityTypes.Attachment);
...
class ContentFetchSyncActivity extends ENSyncActivity_1.ENSyncActivity {
    ...
    async runSyncImpl(trc) {
        ...
        const noteIDs = await en_thrift_connector_1.getPendingOfflineNoteIDs(trc, this.context.syncEngine.graphStorage);
