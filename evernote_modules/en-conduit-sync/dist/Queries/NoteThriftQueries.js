...
function addNoteMutators(out, offlineContentStrategy) {
    async function noteCheckForUpdatesResolver(parent, args, context, info) {
        ...
        const auth = conduit_auth_shared_1.decodeAuthData(syncContextMetadata.authToken);
        const noteStoreUrl = syncContextMetadata.sharedNotebookNoteStoreUrl || auth.urls.noteStoreUrl;
        const resp = await conduit_utils_1.withError(SharedNoteSync_1.fetchNote(context.trc, context.comm, auth, en_thrift_connector_1.convertGuidToService(args.id, en_core_entity_types_1.CoreEntityTypes.Note), noteStoreUrl));
        if (resp.err instanceof conduit_utils_1.RetryError) {
            // Even if offline, we should probably try to force an nsync flush, just in case there are waiting documents
            if (args.flushNSync) {
        ...
        if (localNote.version < serviceNote.updateSequenceNum) {
            ...
            await context.db.transactSyncedStorage(context.trc, 'updateNoteOnDemand', async (graphTransaction) => {
                ...
                await en_thrift_connector_1.NoteConverter.convertFromService(context.trc, params, syncContext, serviceNote);
            });
    ...
    out.noteCheckForUpdates = {
        args: conduit_core_1.schemaToGraphQLArgs({ id: 'ID', fetchContent: conduit_utils_1.NullableBoolean, flushNSync: conduit_utils_1.NullableBoolean }),
