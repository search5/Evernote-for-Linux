...
const ENSyncEngine_1 = require("../ENSyncEngine");
...
function genUpdate(trc, thriftComm, token, user, vaultUser) {
    return async (tx) => {
        ...
        async function updateUser(trc1, graphTransaction, userData, isVaultUser, auth) {
            var _a, _b, _c, _d;
            const syncContext = await ENSyncEngine_1.initUserSyncContext(trc1, tx, isVaultUser ? conduit_core_1.VAULT_USER_CONTEXT : conduit_core_1.PERSONAL_USER_CONTEXT, auth);
            const params = {
                trc: trc1,
                graphTransaction,
            ...
            await en_thrift_connector_1.convertUserFromService(trc1, params, syncContext, user, isVaultUser);
            if (isVaultUser || !userData.businessUserInfo) {
                await en_thrift_connector_1.initAccountLimitsNode(trc1, Object.assign(Object.assign({}, params), { thriftComm }), syncContext, auth, userData.accountLimits || undefined);
            }
        }
...
async function activateAccount(context, auth) {
    ...
    if (auth.state === conduit_view_types_1.AuthState.NeedSSO || auth.state === conduit_view_types_1.AuthState.NeedTwoFactor) {
        ...
        await setLoginAuthData(context.comm, context, auth, context.clientCredentials.deviceIdentifier);
    }
}
...
function getAuthPlugin(httpClient, tokenRefreshManager, deviceIdentifier) {
    async function getLoginInfoResolver(_, args, context) {
        ...
        if (validateMUPAndKeys(context)) {
            ...
            return Auth.getLoginInfo(context.trc, context.comm, args.serviceHost, extHost, args.clientName, args.usernameOrEmail, args.tokenPayload);
        }
    }
    ...
    async function loginResolver(_, args, context) {
        var _a;
        if (validateMUPAndKeys(context)) {
            const auth = await Auth.login(context.trc, context.comm, args.serviceHost, args.externalServiceHost || args.serviceHost, Object.assign(Object.assign({}, context.clientCredentials), { usernameOrEmail: args.email, password: args.password }), (_a = args.allowFacadeAsPersonal) !== null && _a !== void 0 ? _a : false);
    ...
    async function loginWithCookiesResolver(_, args, context) {
        ...
        const auth = await loginWithCookies(context, context.comm, args.serviceHost, args.userSlot, deviceIdentifier);
        if (!auth) {
    ...
    async function loginWithSplitTokensResolver(_, args, context) {
        ...
        if (validateMUPAndKeys(context)) {
            ...
            let auth2 = null;
            try {
                auth1 = await Auth.login(context.trc, context.comm, args.serviceHost, args.externalServiceHost || args.serviceHost, Object.assign(Object.assign({}, context.clientCredentials), { exchangeToken: args.token1 }), false);
            }
            catch (error) {
                auth1 = await retryLoginAfterLegacyError(context.trc, context.comm, args.serviceHost, Object.assign(Object.assign({}, context.clientCredentials), { exchangeToken: args.token1 }), error);
            }
            if (args.token2) {
                try {
                    auth2 = await Auth.login(context.trc, context.comm, args.serviceHost, args.externalServiceHost || args.serviceHost, Object.assign(Object.assign({}, context.clientCredentials), { exchangeToken: args.token2 }), false);
                }
                catch (error) {
                    auth2 = await retryLoginAfterLegacyError(context.trc, context.comm, args.serviceHost, Object.assign(Object.assign({}, context.clientCredentials), { exchangeToken: args.token2 }), error);
                }
            }
            if (auth2) {
                ...
                await context.multiUserProvider.setCurrentUser(context.trc, loginData2.userID, false, auth2.state === conduit_view_types_1.AuthState.Authorized ? {
                    noSync: true,
                    update: auth2.token ? genUpdate(context.trc, context.comm, auth2.token, auth2.user, auth2.vaultUser) : undefined,
                } : undefined);
                await context.multiUserProvider.setUserInfo(context.trc, {
                ...
                await context.multiUserProvider.setCurrentUser(context.trc, loginData1.userID, false, auth1.state === conduit_view_types_1.AuthState.Authorized ? {
                    noSync: false,
                    update: auth1.token ? genUpdate(context.trc, context.comm, auth1.token, auth1.user, auth1.vaultUser) : undefined,
                } : undefined);
                await context.multiUserProvider.setUserInfo(context.trc, {
                ...
                await setLoginAuthData(context.comm, context, auth1, context.clientCredentials.deviceIdentifier);
            }
    ...
    async function loginWithSSOResolver(_, args, context) {
        if (validateMUPAndKeys(context)) {
            const auth = await Auth.login(context.trc, context.comm, args.serviceHost, args.externalServiceHost || args.serviceHost, Object.assign(Object.assign({}, context.clientCredentials), { usernameOrEmail: args.email, ssoLoginToken: args.ssoLoginToken }), false);
            await activateAccount(context, auth);
            return toGraphQL(auth);
        }
    }
    async function loginWithOAuthResolver(_, args, context) {
        if (validateMUPAndKeys(context)) {
            ...
            const auth = await Auth.login(context.trc, context.comm, args.serviceHost, args.externalServiceHost || args.serviceHost, Object.assign(Object.assign({}, context.clientCredentials), { openIdCredential: {
    ...
    async function loginWithServiceTokenResolver(_, args, context) {
        ...
        if (validateMUPAndKeys(context)) {
            ...
            const auth = await Auth.loginWithServiceToken(context.trc, context.comm, args.serviceHost, args.externalServiceHost || args.serviceHost, args.token || '', slot, (_a = args.allowFacadeAsPersonal) !== null && _a !== void 0 ? _a : false);
            if (!auth) {
    ...
    async function loginWithTwoFactorResolver(_, args, context) {
        if (validateMUPAndKeys(context)) {
            const auth = await Auth.loginWithTwoFactor(context.trc, context.comm, args.serviceHost, args.externalServiceHost || args.serviceHost, args.secondFactorTempToken, Object.assign(Object.assign({}, context.clientCredentials), { oneTimeCode: args.oneTimeCode }));
            await activateAccount(context, auth);
            return toGraphQL(auth);
        }
    }
    ...
    async function logoutResolver(_, args, context) {
        ...
        if (context === null || context === void 0 ? void 0 : context.multiUserProvider) {
            ...
            await context.db.forceStopUpsync(); // stop upsync which relies on auth before clearing.
            ...
            const serviceRes = (currentAuth === null || currentAuth === void 0 ? void 0 : currentAuth.token) ? await Auth.invalidateAuthToken(context.trc, currentAuth.token, context.comm, httpClient, (_a = context.clientCredentials) !== null && _a !== void 0 ? _a : null)
                : { success: true };
    ...
    async function userSignupResolver(_, args, context) {
        if (validateMUPAndKeys(context)) {
            ...
            const auth = await Auth.userSignupandLogin(context.trc, context.comm, httpClient, args.serviceHost, args.externalServiceHost || args.serviceHost, Object.assign(Object.assign({}, context.clientCredentials), { usernameOrEmail: args.email, password: args.password, openIdCredential: {
                    tokenPayload: args.tokenPayload,
    ...
    async function napAuthInfoResolver(_, args, context) {
        ...
        const authResult = await Auth.loginWithNAP(context.trc, context.comm, args, context.clientCredentials, httpClient);
        await setLoginAuthData(context.comm, context, authResult, (_a = context.clientCredentials) === null || _a === void 0 ? void 0 : _a.deviceIdentifier);
        return toGraphQL(authResult);
    }
    async function twoFactorAuthSendCodeResolver(_, args, context) {
        if (context) {
            const utilityStore = context.comm.getUtilityStore(`${args.serviceHost}/utility`);
