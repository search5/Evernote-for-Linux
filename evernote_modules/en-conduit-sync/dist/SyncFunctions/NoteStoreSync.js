...
exports.EmptyNoteStoreSyncState = SimplyImmutable.deepFreeze(Object.assign(Object.assign({}, en_thrift_connector_1.EmptySyncStateWithTurbo), { userLastUpdated: null, businessUsersUpdateCount: null, showChoiceScreen: null, currentDevicesUsed: null, uploaded: null }));
...
async function getAndUpdateUser(trc, params, userLastUpdated) {
    ...
    const user = await SyncHelpers_1.interruptible(params, utilityStore.getUser(trc, params.auth.token));
...
async function getAndUpdateBusinessUsers(trc, params, businessUsersUpdateCount) {
    ...
    const currentUsers = await SyncHelpers_1.interruptible(params, utilityStore.listBusinessUsers(trc, params.auth.token, businessUserFilter));
    const businessProfiles = currentUsers.map(user => en_thrift_connector_1.profileFromUserProfile(user, true));
    await params.syncEngine.transact(trc, 'updateBusinessUsers', async (graphTransaction) => {
        ...
        await ChunkConversion_1.processSyncUpdates(trc, converterParams, params.syncContext, en_thrift_connector_1.ProfileConverter, undefined, 0, businessProfiles);
...
async function getAndProcessRemoteSyncState(trc, params, isCatchup) {
    const noteStore = params.comm.getNoteStore(params.auth.urls.noteStoreUrl);
...
async function updateLocalStateAndAccountCounts(trc, params, transactionName, syncStateUpdates, accountCountUpdates) {
    ...
    await params.syncEngine.transact(trc, transactionName, async (graphTransaction) => {
        ...
        const converterParams = await SyncHelpers_1.getConverterParamsFromSyncParams(trc, graphTransaction, params);
        const serviceParams = Object.assign(Object.assign({}, converterParams), { thriftComm: params.comm });
        await en_thrift_connector_1.updateAccountLimitsNode(trc, serviceParams, params.isVault ? conduit_core_1.VAULT_USER_CONTEXT : conduit_core_1.PERSONAL_USER_CONTEXT, accountCountUpdates);
    });
}
...
async function syncCurrentNoteSnippets(trc, params) {
    ...
    await ChunkConversion_1.fetchAndCacheSnippets(trc, params.comm, params.auth, noteGuids, (innerTrc, name, cb) => {
...
async function syncAllNotesMetadata(trc, params, inTrash) {
    ...
    let offset = (await SyncHelpers_1.getLocalSyncState(trc, params, en_thrift_connector_1.EmptySyncState)).lastUpdateCount;
    while (true) {
        const d0 = Date.now();
