...
async function createSharedNoteSyncContextMetadata(trc, graphTransaction, noteGuid, syncContext, ownerId, encodedAuth, noteStoreUrl, syncStatePath) {
    ...
    if (syncStatePath) {
        await graphTransaction.replaceSyncState(trc, syncStatePath, en_thrift_connector_1.EmptySyncState);
    }
}
...
async function syncSharedNote(trc, params, noteStoreUrl, sharedNoteGuid, sharedObjectOwnerId) {
    ...
    const note = await SyncHelpers_1.interruptible(params, fetchNote(trc, params.comm, params.auth, sharedNoteGuid, noteStoreUrl));
    if (note) {
        ...
        const noteID = en_thrift_connector_1.convertGuidFromService(sharedNoteGuid, en_core_entity_types_1.CoreEntityTypes.Note);
