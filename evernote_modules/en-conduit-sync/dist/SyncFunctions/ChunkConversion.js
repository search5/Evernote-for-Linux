...
const CHUNK_CONVERTERS = [
    { converter: en_thrift_connector_1.WorkspaceConverter, expunged: 'expungedWorkspaces', updated: 'workspaces' },
    { converter: en_thrift_connector_1.NotebookConverter, expunged: 'expungedNotebooks', updated: 'notebooks' },
    { converter: en_thrift_connector_1.TagConverter, expunged: 'expungedTags', updated: 'tags' },
    { converter: en_thrift_connector_1.SavedSearchConverter, expunged: 'expungedSearches', updated: 'searches' },
    ...
    for (const chunk of guidChunks) {
        ...
        await transact(trc, 'cacheSnippets', async (tx) => {
            for (const guid of chunk) {
                ...
                const nodeRef = { id: en_thrift_connector_1.convertGuidFromService(guid, en_core_entity_types_1.CoreEntityTypes.Note), type: en_core_entity_types_1.CoreEntityTypes.Note };
    ...
    while (!isChunkProcessed) {
        ...
        isChunkProcessed = await params.syncEngine.transact(trc, 'convertSyncChunk', async (graphTransaction) => {
            ...
            if (chunk.preferences && chunk.preferences.preferences) {
                ...
                await processSyncUpdates(trc, converterParams, params.syncContext, en_thrift_connector_1.ShortcutConverter, null, expungeVersion, [pref]);
                delete chunk.preferences;
            }
            let expungedLinkedNbs = await en_thrift_connector_1.getExpungedLinkedNotebooks(trc, graphTransaction, chunk.notebooks) || [];
            expungedLinkedNbs = chunk.expungedLinkedNotebooks ? chunk.expungedLinkedNotebooks.concat(expungedLinkedNbs) : expungedLinkedNbs;
            const { partialNbsToAdd, partialNbsToRemove } = await en_thrift_connector_1.processLinkedNotebooksForPartialNotebooks(trc, graphTransaction, params.syncEngine.getBusinessID(), chunk.linkedNotebooks, expungedLinkedNbs);
            if (partialNbsToAdd) {
...
async function convertMessageSyncChunk(trc, params, chunk, syncTime) {
    ...
    await params.syncEngine.transact(trc, 'processShareUpdates', async (graphTransaction) => {
        ...
        for (const update of idsUpdates) {
            await en_thrift_connector_1.addNewIdentity(trc, converterParams, params.syncContext, update);
        }
        for (const update of messageUpdates) {
            await en_thrift_connector_1.MessageConverter.convertFromService(trc, converterParams, params.syncContext, update);
        }
        for (const update of threadUpdates) {
            await en_thrift_connector_1.ThreadConverter.convertFromService(trc, converterParams, params.syncContext, update);
        }
        await en_thrift_connector_1.ThreadConverter.updateMessageMaxForThreads(trc, converterParams, params.syncContext, messageUpdates);
        for (const update of shareUpdates) {
            if (update.senderId === params.personalUserID) {
                // shared by the current user, ignore it
                continue;
            }
            await en_thrift_connector_1.addNewShare(trc, converterParams, params.syncContext, update.attachment.guid, update.senderId, update.sentAt, update.attachment);
        }
