...
async function processUpdates(trc, params, betaFeatures, lastUpdateCount) {
    await params.syncEngine.transact(trc, 'processBetaFeaturesUpdates', async (graphTransaction) => {
        ...
        await ChunkConversion_1.processSyncUpdates(trc, converterParams, params.syncContext, en_thrift_connector_1.BetaFeatureConverter, undefined, 0, betaFeatures);
        const update = {
            lastUpdateCount,
        };
...
async function syncBetaFeature(trc, params, betaFeaturesIDs) {
    ...
    const existingBetaFeatureIDs = existingBetaFeatureRefs.map(ref => parseInt(en_thrift_connector_1.convertGuidToService(ref.id, en_core_entity_types_1.CoreEntityTypes.BetaFeature), 10));
