...
const ReadValidationOptsFillDefaults = {
    ...
    fillDefaults: true,
    // important to do this for enum-upgrade scenarios (old version of client may read data published by a newer backend, with additional enum values)
    allowUnknownEnumValues: true,
};
function validateSchemaAndPopulateDefaults(obj, schema) {
    for (const key in schema) {
        if (key in obj) {
            validateSchemaType(schema[key], key, obj[key], ReadValidationOptsFillDefaults);
        }
        else {
...
function validateSchemaType(fieldType, path, value, opts = {
    objectStrictCheck: true,
    deepObjectStrictCheck: true,
    fillDefaults: false,
    ...
    if (fieldTypeIsEnum(fieldType)) {
        if (opts.allowUnknownEnumValues) {
            if (index_1.getTypeOf(index_1.firstStashEntry(fieldType.enumMap)) !== valType) {
                throw new Error(`Field value does not match schema: [${path}]`);
            }
        }
        else if (!Object.values(fieldType.enumMap).includes(value)) {
            throw new Error(`Field value does not match schema: [${path}]`);
        }
        return;
    }
    if (fieldTypeIsNullable(fieldType)) {
        ...
        validateSchemaType(fieldType.nullableType, path, value, opts);
        return;
    }
    const subopts = opts.deepObjectStrictCheck === opts.objectStrictCheck ? opts : Object.assign(Object.assign({}, opts), { objectStrictCheck: opts.deepObjectStrictCheck });
    if (fieldTypeIsList(fieldType)) {
        if (!Array.isArray(value)) {
            throw new Error(`Field value does not match schema: [${path}]`);
        }
        for (let i = 0; i < value.length; ++i) {
            validateSchemaType(fieldType.listType, path.concat('.', i.toString()), value[i], subopts);
        }
        return;
    }
    if (fieldTypeIsMap(fieldType)) {
        ...
        for (const key in value) {
            validateSchemaType(mapType, path.concat('.', key), value[key], subopts);
        }
        return;
    }
    if (fieldTypeIsStruct(fieldType)) {
        ...
        for (const key in fieldType.fields) {
            if (opts.fillDefaults && !(key in value)) {
                value[key] = getSchemaTypeDefaultValue(fieldType.fields[key]);
            }
            else {
                validateSchemaType(fieldType.fields[key], path.concat('.', key), value[key], subopts);
            }
        }
        if (!opts.objectStrictCheck) {
            return;
        }
        for (const key in value) {
