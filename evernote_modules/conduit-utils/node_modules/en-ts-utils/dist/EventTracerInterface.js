...
class EventTraceControllerBase {
    ...
    sanitizeArgsImpl(obj) {
        const type = _1.getTypeOf(obj);
        if (type === 'array') {
            for (let i = 0; i < obj.length; i++) {
                obj[i] = this.sanitizeArgsImpl(obj[i]);
            }
        }
        else if (type === 'object') {
            for (const k of Object.keys(obj)) {
                const key = k.toLowerCase();
                if (key.includes('password') || key.includes('token') || key.includes('bearer')) {
                    obj[k] = 'redacted';
                }
                else {
                    obj[k] = this.sanitizeArgsImpl(obj[k]);
                }
            }
        }
        else if (type === 'string') {
            return this.enableVerboseTracing ? obj : js_md5_1.default(obj);
        }
        return obj;
    }
    sanitizeArgs(vars) {
        try {
            return this.sanitizeArgsImpl(simply_immutable_1.cloneMutable(vars));
        }
        catch (e) {
            _1.logger.warn('EventTraceController: sanitize args error ', e);
            return;
        }
    }
    recordTraceEvents(events) {
        for (const ev of events) {
            ...
            if ((ev.ph === 'B' || ev.ph === 'b') && ev.args) {
                ev.args = this.sanitizeArgs(ev.args);
            }
