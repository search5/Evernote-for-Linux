...
async function genNoteCreate(trc, ctx, params, plan) {
    ...
    */
    ...
    if (params.notesToTrash) {
        // moves notes to trash at the end.
        plan.lateOps = plan.lateOps || [];
        const permContext = new ShareUtils_1.MutationPermissionContext(trc, ctx);
        for (const noteToTrash of params.notesToTrash) {
            const policy = await CommandPolicyRules_1.commandPolicyOfNote(noteToTrash, permContext);
            if (policy.canMoveToTrash) {
                await genNoteMoveToTrashOps(trc, ctx, { id: noteToTrash, type: EntityConstants_1.CoreEntityTypes.Note }, plan.lateOps);
            }
            else {
                conduit_utils_1.logger.warn(`genNoteCreate permission error: cannot move note ${noteToTrash} to trash. Skipping.`);
            }
        }
    }
    return { noteID, owner };
