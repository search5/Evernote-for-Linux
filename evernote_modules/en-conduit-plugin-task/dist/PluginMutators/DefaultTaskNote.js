...
async function resolver(parent, argsIn, context) {
    ...
    if (args.noteContent && (!args.noteContent.includes('</en-note>') || !args.noteContent.includes('<en-note>'))) {
        throw new conduit_utils_1.InvalidParameterError('note content is not valid!');
    }
    if (!args.defaultTaskNoteLabel) {
        throw new conduit_utils_1.InvalidParameterError('defaultTaskNoteLabel can not be empty');
    }
    ...
    let existingDefaultTaskNote;
    if (existingTaskUserSettings && defaultTaskNoteList.length) {
        // trying to ignore invalid edges
        for (const defaultTaskNote of defaultTaskNoteList) {
            if (defaultTaskNote && defaultTaskNote.id) {
                const existingNote = await context.db.getNode(context, { type: en_core_entity_types_1.CoreEntityTypes.Note, id: defaultTaskNote.id });
                if (existingNote && !existingNote.NodeFields.deleted) {
                    defaultTaskNoteID = defaultTaskNote.id;
                    existingDefaultTaskNote = existingNote;
                    continue;
                }
            }
        }
    }
    if (!existingDefaultTaskNote) {
        ...
        const noteContent = args.noteContent ? args.noteContent.replace('</en-note>', `${taskGroupBlock}</en-note>`)
            : `<?xml version="1.0" encoding="UTF-8"?><!DOCTYPE en-note SYSTEM "http://xml.evernote.com/pub/enml2.dtd"><en-note>${taskGroupBlock}</en-note>`;
        const newNote = await context.db.runMutator(context.trc, 'noteCreate', {
            untitledNoteLabel: 'Untitled',
