...
function getENMonetizationPlugin(thriftComm) {
    const getPaywallStateResolver = async (parent, args = {}, context) => {
        ...
        return {
            state: await EnMonetizationService_1.getPaywallState(context.trc, thriftComm, authData, { clientType }),
        };
    };
    const createDeviceSyncResolver = async (parent, args = {}, context) => {
        ...
        const authData = en_thrift_connector_1.decodeAuthData(await conduit_core_1.retrieveAuthorizedToken(context));
        return {
            state: await EnMonetizationService_1.createDeviceSync(context.trc, thriftComm, authData),
        };
    };
    return {
        name: 'ENMonetization',
