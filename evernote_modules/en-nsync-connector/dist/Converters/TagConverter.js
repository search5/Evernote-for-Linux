...
const getTagNodesAndEdges = async (trc, instance, context) => {
    ...
    const tag = Object.assign(Object.assign({}, initial), { type: en_data_model_1.CoreEntityTypes.Tag, NodeFields: {}, inputs: {
            refs: {},
            parent: {},
        }, outputs: {
            children: {},
    ...
    const currentTag = await context.tx.getNode(trc, null, { type: en_data_model_1.CoreEntityTypes.Tag, id: tag.id });
    const parentEdge = conduit_utils_1.firstStashEntry(currentTag === null || currentTag === void 0 ? void 0 : currentTag.inputs.parent);
    if (parentEdge) {
        const currentParentID = parentEdge.srcID;
        if (instance.parentGuid !== currentParentID) {
            // remove existing parent
            edgesToDelete.push({
                // srcID: currentTag.inputs.parent.id, srcType: CoreEntityTypes.Tag, srcPort: 'children',
                dstID: tag.id, dstType: en_data_model_1.CoreEntityTypes.Tag, dstPort: 'parent',
            });
        }
    }
    if (instance.parentGuid) {
        // create new edge if parentGuid is present
        edgesToCreate.push({
            srcID: instance.parentGuid,
            srcType: en_data_model_1.CoreEntityTypes.Tag, srcPort: 'children',
            dstID: tag.id, dstType: en_data_model_1.CoreEntityTypes.Tag, dstPort: 'parent',
        });
    }
