...
function tagFromService(serviceData) {
    const tag = {
        id: Converters_1.convertGuidFromService(serviceData.guid, en_data_model_1.CoreEntityTypes.Tag),
    ...
    if (serviceData.parentGuid) {
        conduit_storage_1.addInputEdgeToNode(tag, 'parent', {
            id: Converters_1.convertGuidFromService(serviceData.parentGuid, en_data_model_1.CoreEntityTypes.Tag),
            port: 'children',
            type: en_data_model_1.CoreEntityTypes.Tag,
        });
    }
...
class TagConverterClass {
    constructor() {
        this.nodeType = en_data_model_1.CoreEntityTypes.Tag;
    }
    ...
    async deleteFromService(trc, params, syncContext, ids) {
        ...
        const noteStore = params.thriftComm.getNoteStore(auth.urls.noteStoreUrl);
        for (const id of ids) {
            const serviceGuid = Converters_1.convertGuidToService(id, en_data_model_1.CoreEntityTypes.Tag);
    ...
    async updateToService(trc, params, syncContext, tagID, diff) {
        const tagNode = await params.graphTransaction.getNode(trc, null, { id: tagID, type: en_data_model_1.CoreEntityTypes.Tag });
        if (!tagNode) {
        ...
        const noteStore = params.thriftComm.getNoteStore(auth.urls.noteStoreUrl);
        const serviceData = {
            guid: Converters_1.convertGuidToService(tagID, en_data_model_1.CoreEntityTypes.Tag),
        };
        ...
        if (Object.keys(serviceData).length > 1) {
            ...
            serviceData.parentGuid = (parent && parent.srcType === en_data_model_1.CoreEntityTypes.Tag) ? Converters_1.convertGuidToService(parent.srcID, en_data_model_1.CoreEntityTypes.Tag) : null;
        ...
        if (parentChanges) {
            ...
            for (const edge of parentChanges.creates) {
                ...
                if (type === en_data_model_1.CoreEntityTypes.Tag) {
                    serviceData.parentGuid = Converters_1.convertGuidToService(id, en_data_model_1.CoreEntityTypes.Tag);
                }
            }
        }
        if (Object.keys(serviceData).length > 1) {
...
__decorate([
    conduit_utils_1.traceAsync(en_data_model_1.CoreEntityTypes.Tag)
