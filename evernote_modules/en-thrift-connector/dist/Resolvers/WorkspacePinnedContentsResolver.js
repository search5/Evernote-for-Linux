...
function WorkspacePinnedContentsResolver(thriftComm) {
    async function resolveWorkspacePinnedContents(workspaceID, context) {
        ...
        const res = await context.db.getNodeCachedField(context, { id: workspaceID, type: en_data_model_1.CoreEntityTypes.Workspace }, WorkspaceConverter_1.PINNED_CONTENTS_FIELD_NAME, async (node, syncContext) => {
            ...
            const pinnedContentsFromServer = await utilityStore.listPinnedContent(context.trc, auth.token, Converters_1.convertGuidToService(node.id, en_data_model_1.CoreEntityTypes.Workspace));
    ...
    return {
        'Workspace.pinnedContents': autoResolverData => {
            const configMap = {};
            for (const field in en_data_model_1.pinnedContentDef) {
                conduit_core_1.schemaFieldToGraphQL(autoResolverData, configMap, field, en_data_model_1.pinnedContentDef[field], 'pinnedContents', [en_data_model_1.CoreEntityTypes.Note, en_data_model_1.CoreEntityTypes.Notebook]);
            }
            return {
