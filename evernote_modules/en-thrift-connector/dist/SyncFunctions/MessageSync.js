...
async function syncMessages(trc, params) {
    var _a;
    const messageStore = params.thriftComm.getMessageStore(params.auth.urls.messageStoreUrl);
    ...
    while (true) {
        ...
        params.setProgress && await params.setProgress(trc, floor);
        const syncAvailable = await SyncHelpers_1.checkIfSyncAvailable(trc, params, SyncHelpers_1.EmptySyncState, chunk.chunkMaxEventId);
        loopCount++;
        if (!syncAvailable) {
